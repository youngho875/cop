...
//	EnableTextWrap(FALSE);
	...
	*/
	SetDelayTime(PPTOOLTIP_TIME_INITIAL, 500);
	SetDelayTime(PPTOOLTIP_TIME_AUTOPOP, 5000);
	SetDelayTime(PPTOOLTIP_TIME_FADEIN, 1000);
	SetDelayTime(PPTOOLTIP_TIME_FADEOUT, 1000);
...
BOOL CPPToolTip::Create(CWnd* pParentWnd, BOOL bBalloon /* = TRUE */) 
{
	...
	//SetDelayTime(PPTOOLTIP_TIME_ANIMATION, 1000);
	...
	case WM_MOUSEMOVE:
		if(pMsg->wParam == MK_SHIFT)
		{
			if ((PPTOOLTIP_HELP == m_nTooltipType) || (PPTOOLTIP_HELP == m_nNextTooltipType))
				...
				double fX,fY;
				CoordMgr->ScreenXY2WorldXY(pMsg->pt.x,pMsg->pt.y,&fX,&fY);
...
void CPPToolTip::OnTimer(UINT_PTR nIDEvent) 
{
	...
	case TIMER_SHOW:
		...
		//CoordMgr->WorldXY2ScreenXY(m_fOriginalX,m_fOriginalY,&ptOriginal.x,&ptOriginal.y);
		
		if ((pt.x != m_ptOriginal.x) || (pt.y != m_ptOriginal.y))
		//if ((pt.x != ptOriginal.x) || (pt.y != ptOriginal.y))
		{
		...
		else if (PPTOOLTIP_STATE_HIDEN == m_nTooltipState)
		{
			...
			// 2007.02.01 ¿˝¥Î¡¬«•∏¶ «•«ˆ«œ∞‘ ºˆ¡§ YHJ
			//Display first step
			PrepareDisplayTooltip(&m_ptOriginal);
			//PrepareDisplayTooltip(&ptOriginal);
			//Fade In effect
