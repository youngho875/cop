...
//////////////////////////////////////////////////////////////////////
//
// Path.cpp: implementation of the CPathFile class.
//
//////////////////////////////////////////////////////////////////////
...
CPathFile::CPathFile(LPCTSTR szPath, BOOL bIsFolderPath, BOOL bHasArguments)
{
	SetPath(szPath, bIsFolderPath, bHasArguments);
}

CPathFile::CPathFile(DWORD dwSpecial)
{
	SetPath(dwSpecial);
}
...
void CPathFile::SetPath(DWORD dwSpecial)
{
	switch (dwSpecial)
	{
	case PATH_CMDLINE:
...
void CPathFile::SetPath(LPCTSTR szPath, BOOL bIsFolderPath, BOOL bHasArguments)
{
	...
	if (bIsFolderPath)
	{
		_sDir = CPathFile::AddBackSlash(_sDir);
		_sDir += szName;
		_sDir = CPathFile::AddBackSlash(_sDir);
	}
	else
...
BOOL CPathFile::IsLocalPath()
{
	return !_sDrive.IsEmpty() && !_bIsRelative;
}

BOOL CPathFile::IsRelativePath()
{
	return _bIsRelative;
}

BOOL CPathFile::IsFilePath()
{
	return !_sFileTitle.IsEmpty();
}

BOOL CPathFile::ExistFile()
{
...
BOOL CPathFile::ExistLocation()
{
	return PathFileExists(GetLocation());
}

CString CPathFile::GetAbsolutePath(LPCTSTR szBaseFolder)
{
	...
	sFullPath = CPathFile::AddBackSlash(sFullPath);
	sFullPath += GetPath();
...
CString CPathFile::GetRelativePath(LPCTSTR szBaseFolder)
{
	if (IsRelativePath()) return sCEmptyString;
...
CString CPathFile::GetPath(BOOL bAppendArgs, BOOL bOriginal)
{
	CString sPath;
...
CString	CPathFile::GetShortPath()
{
...
CString CPathFile::GetDrive()
{
	return _sDrive;
}

CString	CPathFile::GetDriveLabel(BOOL bPCNameIfNetwork)
{
...
int	CPathFile::GetDirCount()
{
	FillDirArray();
	return _aDir.GetSize();
}

CString CPathFile::GetDir(int nIndex)
{
	if (nIndex < 0)
		return _sDir;
...
CString	CPathFile::GetLocation()
{
	return _sDrive + GetDir();
}

CString CPathFile::GetFileTitle()
{
	return _sFileTitle;
}

CString CPathFile::GetFileName()
{
	return _sFileTitle + GetExtension();
}

CString CPathFile::GetExtension()
{
...
CString CPathFile::GetExtName()
{
	return _sExtName;
}

int CPathFile::GetArgCount()
{
	FillArgArray();
	return _aArgs.GetSize();
}

CString CPathFile::GetArgument(int nIndex, BOOL bGetFlag)
{
	if (nIndex < 0)
	{
		if (_sArgs.IsEmpty())
...
void CPathFile::SetArguments(LPCTSTR szArgs)
{
	_aArgs.RemoveAll();
	_sArgs = szArgs;
}

void CPathFile::AddSetArgument(LPCTSTR szFlag, LPCTSTR szArgument)
{
	int	nIndex;
...
void CPathFile::RemoveArgument(int nIndex)
{
...
int CPathFile::FindArgument(LPCTSTR szFlag)
{
	bool		bFound = false;
	int			nIndex;
	CArgument	arg;
...
BOOL CPathFile::GetFileSize(__int64 &nSize)
{
	BOOL bResult;
...
BOOL CPathFile::GetFileTime(CTime &time, DWORD dwType)
{
	BOOL bResult;
	FILETIME *pTime = NULL;
...
BOOL CPathFile::FillFileInfoStruct()
{
	HANDLE	hFile;
	BOOL	bResult;
...
CString CPathFile::AddBackSlash(LPCTSTR szFolderPath, BOOL bInverted)
{
...
CString CPathFile::RemoveBackSlash(LPCTSTR szFolderPath)
{
	CString	sResult(szFolderPath);
	int		nLastChar = sResult.GetLength() - 1;
...
CPathFile::operator LPCTSTR ()
{
	_sLPCTSTRPath = GetPath();
	return _sLPCTSTRPath;
}

const CPathFile& CPathFile::operator = (LPCTSTR szPath)
{
	SetPath(szPath);
	return *this;
}

const CPathFile& CPathFile::operator = (CPathFile &ref)
{
	_aArgs.RemoveAll();
