...
int shpDBF::Open( const char * pszFilename, const char * pszAccess )
{
	...
	m_nRecords = nRecords = pabyBuf[4] + pabyBuf[5]*256 + pabyBuf[6]*256*256 + pabyBuf[7]*256*256*256;
	...
	for( iField = 0; iField < nFields; iField++ ) 
	{
		...
		if (iField == 0 )
			...
			m_panFieldOffset[iField] = m_panFieldOffset[iField-1] + m_panFieldSize[iField-1];
    }
		       ...
		       DBFFieldType eType, int nWidth, int nDecimals )
{
	...
	m_panFieldOffset = (int *)SfRealloc( m_panFieldOffset, sizeof(int) * m_nFields );
	m_panFieldSize = (int *)SfRealloc( m_panFieldSize, sizeof(int) * m_nFields );
	m_panFieldDecimals = (int *)SfRealloc( m_panFieldDecimals, sizeof(int) * m_nFields );
	m_pachFieldType = (char *)SfRealloc( m_pachFieldType, sizeof(char) * m_nFields );
...
void* shpDBF::ReadAttribute(int hEntity, int iField, char chReqType)
{
	...
	/* -------------------------------------------------------------------- */
	if( (m_panFieldSize[iField]+1 > nStringFieldLen) || (pszStringField == NULL) )
	{
	...
	/* -------------------------------------------------------------------- */
	strncpy( pszStringField, (const char*)pabyRec+m_panFieldOffset[iField], m_panFieldSize[iField] );
...
DBFFieldType shpDBF::GetFieldInfo(int iField, char * pszFieldName, int * pnWidth, int * pnDecimals)
{
	...
	if (m_pachFieldType[iField] == 'N' || m_pachFieldType[iField] == 'D' ) 
	{
...
int shpDBF::WriteAttribute(int hEntity, int iField, void * pValue )
{
	...
	if (hEntity == m_nRecords ) 
	{
		...
		m_nRecords++;
		for( i = 0; i < m_nRecordLength; i++ ) 
			m_pszCurrentRecord[i] = ' ';
	...
	case 'N':
		...
		{
			sprintf( szFormat, "%%%d.%df", m_panFieldSize[iField], m_panFieldDecimals[iField] );
